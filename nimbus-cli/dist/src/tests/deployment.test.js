...
describe('deployment.ts', () => {
    ...
    it('should deploy API Gateway', async () => {
        ...
        expect(mockExecPromise).toHaveBeenCalledWith(`cdk deploy ApiGatewayStack --require-approval never -c finishedDirPath="${mockFinishedDirPath}"`, { cwd: `${mockCurrentDir}/../nimbus-cdk` });
    });
    ...
    it('should destroy the stack', async () => {
        ...
        expect(mockExecPromise).toHaveBeenCalledWith(`cdk destroy ApiGatewayStack --force -c finishedDirPath="${mockFinishedDirPath}"`, { cwd: `${mockCurrentDir}/../nimbus-cdk` });
    });
});
